 ...
 * Return: nummber of unique nodes in list before a loop
 */
...
size_t print_listint_safe(const listint_t *head)
{
	...
	if (loop_found == 1) /* print up to last node before loop if loop */
	{
	...
	else if (loop_found == 0) /* print regularly up to NULL if no loop */
	{
		tmp = head;
